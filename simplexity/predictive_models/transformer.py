...
class AttentionProfile(pz.nn.Layer):
    """Compute the average attention profile over the target positions relative to the query.

    Only averages over query positions >= horizon.
...
def attention_profile(attention_QK: jax.Array, horizon: int) -> jax.Array:
    """Computes average attention profile over target positions relative to query position.
