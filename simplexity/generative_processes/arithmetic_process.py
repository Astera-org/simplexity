...
class RPNArithmeticProcess(ArithmeticProcess):
    ...
    def random_sub_equation(self, key: chex.PRNGKey, k: int) -> tuple[int, jax.Array]:
        ...
        Creates a valid RPN expression with k operators and k+1 operands.
        The expression is generated by ensuring proper stack balance.
        ...
        # Generate a valid RPN sequence
        # We'll use a simple approach: place operands first, then operators
        # This ensures we always have enough operands on the stack
        operand_idx = 0
        operator_idx = 0

        for i in range(n):
            if operand_idx < k + 1:
                sub_equation = sub_equation.at[i].set(operands[operand_idx])
                operand_idx += 1
            else:
                sub_equation = sub_equation.at[i].set(operators[operator_idx])
                operator_idx += 1
