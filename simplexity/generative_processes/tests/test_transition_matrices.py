...
def validate_transition_matrices(transition_matrices: jnp.ndarray):
    ...
    assert jnp.allclose(sum_over_obs_and_next, 1.0), (
        "Probabilities don't sum to 1 for each current state"
    )
    
    state_transition_matrix = jnp.sum(transition_matrices, axis=0)
    eigenvals = jnp.linalg.eigvals(state_transition_matrix)
    max_eigenval = jnp.abs(eigenvals).max()
...
def test_post_quantum():
    ...
    try:
        ...
        pytest.xfail("Not fully verified")
