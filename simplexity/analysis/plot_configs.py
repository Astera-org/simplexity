...
def generate_pca_2d_config(
    ...
    for step in steps:
        for layer_name in layers:
            ...
            plot_layers.append(LayerConfig(
                name=f"step_{step}__layer_{layer_name}",
                data=DataConfig(
                    source="pca_data",
                    filters=[f"step == {step}", f"layer == '{layer_name}'"]
                ),
                ...
                aesthetics=AestheticsConfig(
                    ...
                    color=ChannelAestheticsConfig(field="point_id", type="quantitative"),
        ...
        for step in steps:
            for layer_name in layers:
                plot_layers.append(LayerConfig(
                    name=f"step_{step}__layer_{layer_name}",
                    data=DataConfig(
                        source="variance_data",
                        filters=[f"step == {step}", f"layer == '{layer_name}'"]
                    ),
                    geometry=GeometryConfig(type="line", props={"width": style.line_width}),
                    aesthetics=AestheticsConfig(
                        x=ChannelAestheticsConfig(field="component", type="quantitative", title=style.component_title),
        ...
        for layer_name in layers:
            ...
            plot_layers.append(LayerConfig(
                ...
                aesthetics=AestheticsConfig(
                    ...
                    color=ChannelAestheticsConfig(value=style.true_color),
                    opacity=ChannelAestheticsConfig(value=style.opacity),
                )
            ))
            ...
            plot_layers.append(LayerConfig(
                ...
                aesthetics=AestheticsConfig(
                    x=ChannelAestheticsConfig(field="x", type="quantitative", title=style.x_title),
                    y=ChannelAestheticsConfig(field="y", type="quantitative", title=style.y_title),
                    color=ChannelAestheticsConfig(value=style.predicted_color),
                    opacity=ChannelAestheticsConfig(value=style.opacity),
                )
            ))
