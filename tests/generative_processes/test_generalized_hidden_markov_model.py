...
def test_generate(model_name: str, request: pytest.FixtureRequest):
    ...
    intermediate_states, intermediate_observations = model.generate(initial_states, keys, sequence_len, False, False)
    ...
    final_states, final_observations = model.generate(intermediate_states, keys, sequence_len, False, False)
...
def test_generate_with_intermediate_states(model_name: str, request: pytest.FixtureRequest):
    ...
    initial_states = jnp.repeat(model.stationary_state[None, :], batch_size, axis=0)
    keys = jax.random.split(jax.random.PRNGKey(0), batch_size)
    intermediate_states, observations = model.generate(initial_states, keys, sequence_len, True, False)
    ...
    final_states, observations = model.generate(last_intermediate_states, keys, sequence_len, True, False)
