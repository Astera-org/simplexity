...
def test_generate(z1r: HiddenMarkovModel):
    ...
    result = z1r.generate(initial_states, keys, sequence_len, False)
    ...
    result = z1r.generate(intermediate_states, keys, sequence_len, False)
...
def test_generate_with_all_states_returns_final_carry(z1r: HiddenMarkovModel):
    """`states` should equal transitioning from the last intermediate state and last observation."""
    batch_size = 4
    sequence_len = 6
    initial_states = jnp.repeat(z1r.normalizing_eigenvector[None, :], batch_size, axis=0)
    keys = jax.random.split(jax.random.PRNGKey(7), batch_size)

    result = z1r.generate(initial_states, keys, sequence_len, True)
    all_states = result["all_states"]
    observations = result["observations"]
    final_states = result["states"]
    assert all_states.shape == (batch_size, sequence_len, z1r.num_states)
    assert observations.shape == (batch_size, sequence_len)
    assert final_states.shape == (batch_size, z1r.num_states)

    expected_final_states = eqx.filter_vmap(z1r.transition_states)(
        all_states[:, -1, :],
        observations[:, -1],
    )
